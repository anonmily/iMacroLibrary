...
var $M = (function(realwindow, window, document, iimPlay, iimGetLastExtract, iimSet){
	...
	var debugOn = function(){ debug = on; }, //TURN ON DEBUGGING MODE
		...
		goto = function(url){ window.location = url; }, //RELOAD PAGE
		...
		setAttribute = function(elem){ 
			...
			//set attribute type
				...
				} else if (attributes.class && attributes.class.split(' ').length === 1) {
	...
	var iMacroLibrary = function(selector, number){
		...
		//GET SELECTED ELEMENT
			...
			if(node.length > 1){
				...
				throw "No matching nodes found for the selector: " + selector + "!";
			}
	...
	var Tag = function(s){ //accepts a spec object
		...
		this.val = function(){
			var currval = this.extract('TXT', true);
			log('CURRENT VALUE IS: ' + currval);
			return currval;
		};
		...
		if(this.type == "SELECT"){ 
			...
			this.isLoaded = function(){
				...
				//check for "Loading" text in current dropdown
	...
	// append globally accessible methods to $M
	iMacroLibrary.tag = Tag;
	iMacroLibrary.debug = {
		on: debugOn,
		off: debugOff
	};
	iMacroLibrary.log = log;
	iMacroLibrary.alert = alert;
	iMacroLibrary.reload = reload;
	iMacroLibrary.goto = goto;
	iMacroLibrary.pause = pause;
	iMacroLibrary.csv = {
		out: exportcsv,
		in: importcsv
	}

	// Return the iMacroLibrary function
	return iMacroLibrary;
